 ...
 * @throws {Error} If start sequence is less than 1
 * @throws {Error} If increment amount is less than 1
 */
Sequence = function(schema, options) {
    ...
    if (options.inc_amount <= 0) {
        throw new Error('Increment amount must be greater than 0');
    }

    if (options.start_seq <= 0) {
        throw new Error('Cannot start sequence at ' + options.start_seq + '. Must be greater than 0');
    }
 ...
 *
 * @method      getIncrementAmount
 * @return      {Number} The number to increment by
 */
Sequence.prototype.getIncrementAmount = function() {
    return this._options.inc_amount;
};
    ...
    CounterSchema = mongoose.Schema(
        {
            ...
            seq: {type:Number, default: this._options.start_seq - 1, required: true}
        },
        {
...
Sequence.prototype._resetCounter = function(id, reference, callback){
    ...
    this._counterModel.updateMany(condition, {$set: {seq: this._options.start_seq - 1}}, null, callback);
};
    ...
    var retriable = function(callback) {
        ...
        this._counterModel.countDocuments({id: id, reference_value: referenceValue}).then((count) => {
            if (count > 0 || (incrementAmount === 1 && this._options.start_seq === 1)) {
                this._counterModel.findOneAndUpdate({ id: id, reference_value: referenceValue },
                ...
                this._counterModel.findOneAndUpdate({ id: id, reference_value: referenceValue },
                    { $set: { seq: this._options.start_seq } },
                    { new: true, upsert: true },
                    function(err, counter) { return callback(null, counter.seq); }
                );
            }
        });
    };
