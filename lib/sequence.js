 ...
 * @param      {Boolean}  incrScope     If set, returns reference fields, used to find old counte
...
Sequence.prototype._getCounterReferenceField = function (doc, incrScope) {
  ...
  if (incrScope) {
    for (var i in this._options.old_reference_fields) {
...
Sequence.prototype._resetCounter = function (id, reference, callback) {
  ...
  this._counterModel.updateMany(condition, {
    $set: {
      seq: 0
    }
  }, null, callback);
    ...
    if (that._options.old_reference_fields) {
      that._counterModel.findOne({
        ...
        if (counter) {
          ...
          that._counterModel.findOne({
            id: oldReferenceID,
            reference_value: that._getCounterReferenceField(doc, true)
          }, function (err, counter) {
            if (err) return callback(err);
            if (!counter) return callback(new Error('Old Counter not found'));
            that._counterModel.findOneAndUpdate({
              id: id,
              reference_value: that._getCounterReferenceField(doc)
            }, {
              seq: counter.seq + 1
            }, {
              new: true,
