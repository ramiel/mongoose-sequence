 ...
 * @param {boolean} [options.exclusive=true] Set False to allow sharing same Sequence with multiple Schemas.
...
Sequence.getInstance = function (schema, options) {
    var sequence = new Sequence(schema, options),
        id = sequence.getId(),
        existsSequence = sequenceArchive.existsSequence(id);
...
Sequence.prototype._createCounterModel = function () {
    var modelName = 'Counter_' + this._options.id;
    ...
    return mongoose.model(modelName, schema, this._options.collection_name);
};
...
Sequence.prototype._setHooks = function () {
    if (this._options.disable_hooks) return;
