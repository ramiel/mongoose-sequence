...
Sequence.getInstance = function(schema, options) {
  ...
  if (!options.duplicate && sequenceArchive.existsSequence(id)){
    throw new Error('Counter already defined for field "'+id+'"');
  }

  if (options.duplicate && !options.super) {
    sequence.enable(options);
    return sequence;
  }
...
* @method     enable
*/
Sequence.prototype.enable = function(options){
  if (options.duplicate) {
    const key = 'Counter_' + this._options.id;
    this._counterModel = sequenceArchive.getSequenceModel(key) || this._createCounterModel();
  } else {
...
* @param      {Boolean}  incrScope     If set, returns reference fields, used to find old counter
* @return     {Array}   An array of strings which represent the value of the
*                       reference
*/
Sequence.prototype._getCounterReferenceField = function(doc, incrScope) {
  var reference = {};

  if (incrScope) {
    for (var index in this._options.old_reference_fields) {
      reference[this._options.reference_fields[index]] = doc[this._options.reference_fields[index]];
    }
    return reference;
  }
...
// });
  const modal = mongoose.model('Counter_' + this._options.id, CounterSchema);
  sequenceArchive.setSequenceModel('Counter_' + this._options.id, modal);
...
Sequence.prototype._setNextCounter = function(doc, callback) {
  ...
  var retriable = function(callback) {
    var that = this;
    var id = that.getId();
    var referenceValue = that._getCounterReferenceField(doc);
    if (that._options.old_reference_fields) {
      that._counterModel.findOne({id : id, reference_value:  referenceValue}, function (err, counter) {
        if (err) return callback(err);
        if (counter) {
          that._counterModel.findOneAndUpdate(
          ...
          const oldReferenceID = `${id}_${ that._options.old_ref_field_suffix|| 'old'}`;
          that._counterModel.findOne({id : oldReferenceID, reference_value:  that._getCounterReferenceField(doc, true)}, function (err, counter) {
            if (err) return callback(err);
            var seq = counter ? counter.seq + 1 : 1;
            that._counterModel.findOneAndUpdate(
                { id: id, reference_value: referenceValue },
                { seq } ,
                { new: true, upsert: true, passRawResult: true },
